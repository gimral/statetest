...
/**
 * A {@link PTransform} that performs a {@link CoGroupByKeyT} on a tuple of tables. A {@link
 * CoGroupByKeyT} groups results from all tables by like keys into {@link CoGbkResult}s, from which
 ...
 *
 * <p>Example of performing a {@link CoGroupByKeyT} followed by a {@link ParDo} that consumes the
 * results:
 *
 * <pre>{@code
 ...
 */
public class CoGroupByKeyT<K>
        extends PTransform<KeyedPCollectionTuple<K>, PCollection<KV<K, RawUnionValue>>> {
    /**
     * Returns a {@code CoGroupByKey<K>} {@code PTransform}.
     *
     * @param <K> the type of the keys in the input and output {@code PCollection}s
     */
    public static <K> CoGroupByKeyT<K> create() {
        return new CoGroupByKeyT<>();
    }

    private CoGroupByKeyT() {}

    @Override
    public PCollection<KV<K, RawUnionValue>> expand(KeyedPCollectionTuple<K> input) {
        if (input.isEmpty()) {
        ...
        for (TaggedKeyedPCollection<K, ?> entry : input.getKeyedCollections()) {
            codersList.add(getValueCoder(entry.getCollection()));
        }
        ...
        for (TaggedKeyedPCollection<K, ?> entry : input.getKeyedCollections()) {
            ...
            PCollection<KV<K, RawUnionValue>> unionTable =
                    makeUnionTable(index, entry.getCollection(), kVCoder);
